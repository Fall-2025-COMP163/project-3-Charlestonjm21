...
if __name__ == "__main__":
    ...
    try:
        ...
        print(f"  Verified: HP={loaded['health']}, STR={loaded['strength']}, MAG={loaded['magic']}\n")
    ...
    # Test Cleric class (verify magic is 15, not 158)
    try:
        cleric = create_character("TestCleric", "Cleric")
        print(f"✓ Created: {cleric['name']} the {cleric['class']}")
        print(f"  Stats: HP={cleric['health']}, STR={cleric['strength']}, MAG={cleric['magic']}")
        assert cleric['magic'] == 15, "Cleric magic should be 15"
        print("  ✓ Cleric magic stat is correct (15)\n")
    except Exception as e:
        print(f"✗ Cleric test failed: {e}\n")
    
    # Test list_saved_characters
    try:
